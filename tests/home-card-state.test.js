...
describe('Home signed card state colors', () => {
  ...
  it('uses stronger smoothing only for the compact Baro sparkline', () => {
    ...
    expect(home).toMatch(/<Sparkline\s+data=\{shownBattSpark\.data\}\s+color=\{socColor\}\s+lineWidth=\{2\}\s+smoothingAlpha=\{1\}\s+curveSmooth=\{0\.25\}\s+heldUntil=\{shownBattSpark\.heldUntil\}\s+heldLineType="solid"\s*\/>/);
