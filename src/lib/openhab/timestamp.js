export function parseSourceTimestamp(value, nowMs = Date.now()) {
  ...
  if (typeof value === 'number') timestamp = value;
    ...
    const normalizedZone = zone === 'Z' ? zone : zone.replace(/^([+-]\d{2})(\d{2})$/, '$1:$2');
    const year = Number(y), month = Number(mo), day = Number(d);
    const leap = year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0);
    const days = [31, leap ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    if (month < 1 || month > 12 || day < 1 || day > days[month - 1]
      || Number(h) > 23 || Number(mi) > 59 || Number(s) > 59) return null;
    if (normalizedZone !== 'Z' && (Number(normalizedZone.slice(1, 3)) > 23 || Number(normalizedZone.slice(4)) > 59)) return null;
    timestamp = Date.parse(`${y}-${mo}-${d}T${h}:${mi}:${s}.${fraction.padEnd(3, '0').slice(0, 3)}${normalizedZone}`);
  }
