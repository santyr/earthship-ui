 ...
 * requests (Discover AES lithium bank; alternating South/East planter
 * greywater circulation pumps). Canonical rewrite of the live
 * `hex_southoutlet_cycle` rule in the
 ...
 *
 * LastCycleStart is the restart-safe 60-minute start-to-start authority
...
function pumpItems() {
  return [CFG.southPumpItem, CFG.eastPumpItem];
}

// Restart-safe alternation: even local clock hours use South, odd hours use East.
// Because starts are limited to once per hour, each pump is scheduled every two
// hours without requiring another persisted selector item.
function selectedPumpItem() {
  return (now().hour() % 2 === 0) ? CFG.southPumpItem : CFG.eastPumpItem;
}

function pumpLabel(name) {
  if (name === CFG.southPumpItem) return 'south';
  if (name === CFG.eastPumpItem) return 'east';
  return name;
}

function activePumpItems() {
  return pumpItems().filter((name) => state(name) === 'ON');
}
...
function safeOffAllPumps() {
  for (const name of pumpItems()) {
    try {
      if (state(name) !== 'OFF') command(name, 'OFF');
    } catch {
      // Best-effort redundant OFF; the next evaluation retries on faults.
    }
  }
}
...
function beginCycle({
  isManual, request, ledger, voltage, soc, mode, invocationToken, pumpItem,
}) {
  const startAt = nowInstantText();
  const pump = pumpItem || selectedPumpItem();
  const pumpName = pumpLabel(pump);

  // Defensive interlock: never intentionally energize both greywater pumps.
  safeOffAllPumps();
  ...
  actions.ScriptExecution.createTimer(now().plusSeconds(CFG.cycleMs / 1000), () => {
    ...
    try {
      ...
      safeOff(actuator);
      // If an unexpected second pump was energized externally, force it off too.
      safeOffAllPumps();
  ...
  if (activePumps.length > 0) {
    if (cache.shared.get(BUSY_KEY) !== null && activePumps.length === 1) {
      status('cycle_active', {
        voltage: voltage.toFixed(2),
        soc: soc.toFixed(1),
        pump: pumpLabel(activePumps[0]),
      });
      return;
    }
    // Orphaned pump, or both pumps ON, with no valid single live cycle -> force
    // both OFF in this same evaluation.
    forceOff(
      activePumps.length > 1 ? 'multiple_pumps_on' : 'orphan_outlet_off',
      {
        voltage: voltage.toFixed(2),
        soc: soc.toFixed(1),
        pumps: activePumps.map(pumpLabel).join('+'),
      },
    );
    return;
  }
