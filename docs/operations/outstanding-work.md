...
| Task 18: Bandit thresholds | Deferred per its explicit prerequisite instruction. Immutable decision/result capture and completed-night assessment now exist; live trough outcomes remain zero on September 20. Records explicitly forbid bandit eligibility and do not assess action attribution. This supersedes the earlier missing-table finding. | Verify the first natural completed assessment (earliest ordinary run September 22 06:40 MDT, coverage permitting), then approve reward/attribution and bounded tuning design. Forecast residuals alone are not advisory benefit or DM utility. Keep current 95/92/90 F advisory and 30% DM thresholds unchanged. |
