...
unchanged. Four Thing definitions differed at the raw comparison: 33
System Info/TP-Link channel metadata entries and 72 clone-only System Info
storage channels generated by the isolated mount layout. A narrow post-run
classifier checked the private source/restored definitions and found **zero
material Thing drift**; Item, Rule and Link definition mismatch lists were
empty. All three uninitialized clone Rules were independently verified as
the same intentionally disabled legacy schedules in production. Future
integrated runs now fail their qualified-boot gate on unexpected definition
drift, missing source channels, unapproved extra channels or uninitialized
active Rules; nine focused restore tests pass. The test is a qualified
