...
The tablet uses the household LAN URL in Chrome/WebView. A user-level systemd
service keeps the Vite server running. Time-of-day auto-dim (console-style
"auto brightness") uses the astro sun phase item rather than the clock.
...
- **Stack**: Svelte + Vite + Tailwind; ECharts (custom console theme) for
  charts. Single client-side application, no SSR.
- **Serving**: the Vite development server is the intentional household
  production server at `http://192.168.1.161:5190/`. It runs from this checkout
  through `earthship-ui.service`, a user-level systemd unit, with
  `--host 0.0.0.0 --port 5190 --strictPort`. There is no nginx, second static
  server, HTTPS/PWA release origin, or immutable release directory.
- **Runtime safety mode**: the unit provides an explicit `RELEASE_MODE`.
  Missing or unknown values fail closed to maintenance behavior. Vite's own
  development-mode flag never decides whether a control may submit a command.
- **Updates and rollback**: verify the working tree, restart the user service,
  and read back its status and HTTP response. Roll back with Git to the last
  verified commit and restart the same service.
...
- **Repo**: `github.com/santyr/earthship-ui` — **private**, under Sat's personal
  `santyr` account (NOT the lightning-goats org). Local clone at
  `~/earthship-ui`. Secrets are never committed; the browser uses the
  same-origin Vite `/rest` proxy.
...
- Playwright smoke at 1340×800 and laptop viewports at 1280×720 or larger:
  every tile renders, no overflow at reference resolution, controls
  confirm-hold works.
...
6. Install and enable the user-level Vite service; verify automatic restart,
   LAN access, auto-dim, and the fail-closed runtime safety mode. MainUI stays
   available for administration.

## Out of scope (v1)

Phone-optimized layouts beyond reflow, PWA/offline packaging, nginx or another
reverse proxy, HTTPS for the household UI, immutable release infrastructure,
