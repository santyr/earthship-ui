...
migration.NAMES = ('Predicted_PV_Today_kWh', 'Predicted_SoC_Trough_Tomorrow')
migration.ITEM_TYPE = 'Number'
migration.SOURCE = ROOT / 'openhab/file-config/items/forecast-energy.items'
migration.TARGET = Path('/etc/openhab/items/forecast-energy.items')
migration.TIMER = 'forecast-intel.timer'
migration.SERVICE = 'forecast-intel.service'
migration.BACKUP_PREFIX = 'forecast-energy'
RELEASE_READY = False

if __name__ == '__main__':
    if sys.argv[1:] not in (['--check'], ['--apply']):
        raise SystemExit('usage: migrate-forecast-energy-items.py --check|--apply')
