...
export const ITEM_NAME = 'Energy_Analytics_JSON';
...
export const ENERGY_ANALYTICS_ITEM = Object.freeze({
  name: ITEM_NAME,
  type: 'String',
  label: 'Energy analytics summary',
  category: '',
  tags: Object.freeze([]),
  groupNames: Object.freeze([]),
});

const RECEIPT_SCHEMA = 'earthship-energy-analytics-config-receipt/v1';
const LOCK_SCHEMA = 'earthship-energy-analytics-config-lock/v1';
...
function stringArray(value, field) {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== 'string')) {
    throw new Error(`Energy analytics item ${field} must be an array of strings`);
  }
  return [...value];
}
...
function itemConfiguration(item) {
  ...
  if (!item || typeof item !== 'object' || Array.isArray(item)) {
    throw new Error('Energy analytics item configuration must be an object or null');
  }
  if (item.name !== ITEM_NAME) throw new Error('Energy analytics item name mismatch');
  if (typeof item.type !== 'string' || !item.type) {
    throw new Error('Energy analytics item type must be a non-empty string');
  }
  for (const field of ['label', 'category']) {
    if (item[field] !== undefined && typeof item[field] !== 'string') {
      throw new Error(`Energy analytics item ${field} must be a string`);
    }
  }
  return {
    name: ITEM_NAME,
...
export function assertAnalyticsConfigRequest(method, path) {
  const key = `${String(method).toUpperCase()} ${String(path)}`;
  if (!ALLOWED_REQUESTS.has(key)) throw new Error(`Denied OpenHAB request: ${key}`);
}

export function authorizeAnalyticsConfigRequest(method, path, {
  body,
  transaction,
} = {}) {
  assertAnalyticsConfigRequest(method, path);
  const key = `${String(method).toUpperCase()} ${String(path)}`;
  ...
  if (key === `PUT ${ITEM_PATH}`) {
    ...
    if (transaction.operation === 'apply' && equal(body, ENERGY_ANALYTICS_ITEM)) return;
...
export function buildApplyPlan(original) {
  ...
  return [{ method: 'PUT', path: ITEM_PATH, body: clone(ENERGY_ANALYTICS_ITEM) }];
}
...
export function assertReceiptIntegrity(receipt, snapshot) {
  if (!receipt || typeof receipt !== 'object' || Array.isArray(receipt)) {
    throw new Error('Energy analytics config receipt is required');
  }
  if (receipt.schema !== RECEIPT_SCHEMA
      || !['open', 'closed'].includes(receipt.state)
      || receipt.itemName !== ITEM_NAME) {
    throw new Error('Energy analytics config receipt identity mismatch');
  }
  if (!Array.isArray(receipt.closures ?? [])
    ...
    throw new Error('Energy analytics config receipt closure evidence mismatch');
  }
  const openPhases = [
  ...
  if (receipt.state === 'open'
    ...
    throw new Error('Energy analytics config receipt open state mismatch');
  }
  if (receipt.state === 'closed') {
    ...
    if (!['desired', 'rolled-back'].includes(receipt.phase)
      ...
      throw new Error('Energy analytics config receipt closed terminal mismatch');
    }
  }
  if (!/^[a-f0-9]{64}$/.test(receipt.checksum ?? '')) {
    throw new Error('Energy analytics config receipt checksum is missing');
  }
  if (digest(receipt) !== receipt.checksum) {
    throw new Error('Energy analytics config receipt checksum mismatch');
  }
  if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot)
      || Object.keys(snapshot).length !== 1 || !Object.hasOwn(snapshot, 'item')) {
    throw new Error('Energy analytics config snapshot shape mismatch');
  }
  const normalized = { item: itemConfiguration(snapshot.item) };
  if (!equal(snapshot, normalized)) throw new Error('Energy analytics config snapshot shape mismatch');
  if (!/^[a-f0-9]{64}$/.test(receipt.snapshotDigest ?? '')
      || digest(snapshot) !== receipt.snapshotDigest) {
    throw new Error('Energy analytics config snapshot digest mismatch');
  }
}
...
async function acquireReceiptLock(receiptDir, operation, now) {
  ...
  try {
    ...
    if (error?.code === 'EEXIST') {
      ...
      throw new Error(
        `Energy analytics config receipt is busy; inspect stale lock ${LOCK_FILENAME} digest ${detail}`,
      );
    }
    throw error;
  }
  ...
  return async () => {
    ...
    if (current.schema !== LOCK_SCHEMA || current.nonce !== lock.nonce) {
      throw new Error('Energy analytics config lock ownership changed; refusing blind deletion');
    }
...
async function readTransaction(receiptDir) {
  ...
  try {
    ...
    throw new Error(`Energy analytics config receipt or snapshot unavailable: ${error.message}`);
  }
...
async function executeOperation(request, operation, transaction) {
  ...
  authorizeAnalyticsConfigRequest(operation.method, operation.path, {
    body: options.body,
    transaction,
  });
...
async function snapshotTransactionUnlocked({ request, receiptDir, now } = {}) {
  ...
  if (await exists(receiptPath) || await exists(snapshotPath)) {
    throw new Error('Energy analytics config receipt directory is not empty');
  }
...
function assertOpenPhase(receipt, command, allowed) {
  if (receipt.state !== 'open') {
    throw new Error(`Cannot ${command} energy analytics item from receipt state ${receipt.state}`);
  }
  if (!allowed.includes(receipt.phase)) {
    throw new Error(`Cannot ${command} energy analytics item from receipt phase ${receipt.phase}`);
  }
}
...
async function applyTransactionUnlocked({ request, receiptDir, now } = {}) {
  ...
  if (digest({ item: live }) !== receipt.snapshotDigest) {
    throw new Error('Live Energy_Analytics_JSON configuration drifted from the captured pre-state');
  }
  ...
  if (!equal(verified, ENERGY_ANALYTICS_ITEM)) {
    throw new Error('Energy_Analytics_JSON desired configuration verification failed');
  }
...
async function verifyTransactionUnlocked({ request, receiptDir } = {}) {
  ...
  if (['applied', 'desired'].includes(receipt.phase)) {
    return {
      ok: equal(live, ENERGY_ANALYTICS_ITEM),
      expected: 'desired',
      phase: receipt.phase,
    };
  }
  ...
  throw new Error(`Cannot verify energy analytics item from receipt phase ${receipt.phase}`);
}
...
async function rollbackTransactionUnlocked({ request, receiptDir, now } = {}) {
  ...
  if (receipt.state === 'closed') {
    if (receipt.phase !== 'desired') {
      throw new Error(`Cannot rollback energy analytics item from closed phase ${receipt.phase}`);
    }
    live = await readLiveItem(request);
    if (!equal(live, ENERGY_ANALYTICS_ITEM)) {
      throw new Error('Live Energy_Analytics_JSON configuration drifted from closed desired state');
    }
    receipt = await writeReceipt(transaction.receiptPath, receipt, {
      now,
      state: 'open',
  ...
  if (!equal(live, ENERGY_ANALYTICS_ITEM)) {
    throw new Error('Live Energy_Analytics_JSON configuration drifted from receipt-owned states');
  }
  ...
  if (!equal(restored, transaction.snapshot.item)) {
    throw new Error('Energy_Analytics_JSON rollback verification failed');
  }
  return writeReceipt(transaction.receiptPath, receipt, { now, phase: 'rolled-back' });
}
...
async function closeTransactionUnlocked({ request, receiptDir, now } = {}) {
  ...
  const expected = receipt.phase === 'desired' ? ENERGY_ANALYTICS_ITEM : snapshot.item;
  const live = await readLiveItem(request);
...
async function rehearseTransactionUnlocked({ receiptDir, now } = {}) {
  ...
  const isolated = await mkdtemp(join(tmpdir(), 'energy-analytics-rehearse-'));
...
export function createRestClient({
  ...
  return async function request(method, path, options = {}) {
    const hasBody = Object.hasOwn(options, 'body');
    authorizeAnalyticsConfigRequest(method, path, {
...
function usage() {
  return 'Usage: energy-analytics-config.mjs snapshot|plan|rehearse|apply|verify|rollback|settle|close|inspect-lock --receipt-dir PATH';
}
